...
public class VerificationLabelsAfterIntakeCreationPage extends PageBase {
	...
	@FindBy(xpath="(//*[text()='Allegation Reports']//..//..//a)[last()]")
	WebElement allegationReport;
	...
	@FindBy(xpath="//*[text()='Close Allegation Reports']//..//lightning-primitive-icon")
    WebElement closeAllegation;
	...
	public void verifyAllegationCasePerson(String AllegationReportIDIntake) throws AcisException, InterruptedException {
		...
		try {
			...
			waitForPageLoadTill(5);
			clickWebElement(allegationReport);
			logger.info("Clicked on allegation tab");
			
			waitForPageLoadTill(5);
			List<WebElement> allegationIDs = driver.findElements(By.xpath("//table/tbody/tr/th/span/a"));
			int totalIncident = allegationIDs.size();

			for (int i = 0; i < totalIncident; i++) {

				System.out.println(allegationIDs.get(i).getText());
				if (allegationIDs.get(i).getText().equalsIgnoreCase(AllegationReportIDIntake)
					) {
	...
	public void verifyCasePersonRelatedList() throws AcisException, InterruptedException {
		...
		try {
			...
			clickWebElemntUsingJavascriptExecutor(closeAllegation);
			logger.info("Closed allegation");
