...
//Khushboo
	...
	@FindBy(xpath="//a[contains(text(),'A-00')]")
	WebElement assessmentId;
	
	@FindBy(xpath="//span[text()='Status']/../../..//input")
	 ...
	 * @throws InterruptedException 
	 */
	public void submitBtn() throws AcisException {
	 ...
	 * @throws InterruptedException 
	 */
	public String fetchAssessmentId() throws AcisException {
		String assessmentID;
		logger.info("Clicking on first record");
		waitForElement(assessmentId);
		assessmentID = assessmentId.getText();
		System.out.println("Created Assessment Case ID " + assessmentID);
		testLoggerInfo("Created Assessment  Case ID " + assessmentID);
		waitForPageLoadTill(5);
		waitForPageLoad();
		return assessmentID;
	}
	
	/*** Select Status Approve
	 * @author pallavi.sanjay.yemle
	 * @throws AcisException 
	 */
	public void statusApprove() throws AcisException {
		logger.info(" Selecting Status Approve");
		waitForElement(statusTextBox);
		clickWebElement(statusTextBox);
		clickWebElement(statusApproved);
		logger.info(" Selected Status Approve");
		waitForPageLoadTill(5);
		waitForPageLoad();
	}
