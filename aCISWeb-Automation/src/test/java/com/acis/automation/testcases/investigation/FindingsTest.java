...
/*
	@Test(priority = 4, description = "S:0107 Investigation -Review safety Assessment records interview records Findings using investigation User", groups = {
			"Simple", "ACIS_Regression", "Khushboo" })
	public void reviewSafteyAssessmentFindingsInvestigator() throws AcisException, InterruptedException {
		try {
			userLogin.loginUserToApplication(testData.get(Constants.LOGINPAGE_SHEET).get("investigatorUserName"),
					testData.get(Constants.LOGINPAGE_SHEET).get("investigatorPassword"));
			home.verfiyHomePage();
			...
			// Record
			recordReview_OmniScriptPage.nextPage();
			findings_OmniScriptPage.reviewSafteyAssessment();
		} catch (Exception e) {
			// TODO Auto-generated catch block
			getStackTrace(e,
					"Failed S:0107 Investigation -Review safety Assessment records interview records Findings using investigation User");
		}
